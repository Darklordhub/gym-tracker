...
public class ExerciseDbExerciseMediaProvider : IExerciseMediaProvider
{
    public string Name => "exercisedb";
    public int Priority => 300;

    private readonly HttpClient _httpClient;
    private readonly ExerciseDbMediaProviderOptions _options;
    private readonly ILogger<ExerciseDbExerciseMediaProvider> _logger;
    private IReadOnlyList<ExerciseDbMediaEntry>? _entries;

    public bool IsEnabled => _options.Enabled && !string.IsNullOrWhiteSpace(_options.BaseUrl);

    public ExerciseDbExerciseMediaProvider(
        HttpClient httpClient,
        IOptions<ExerciseMediaEnrichmentOptions> options,
        ILogger<ExerciseDbExerciseMediaProvider> logger)
    {
        _httpClient = httpClient;
        _options = options.Value.ExerciseDb;
        _logger = logger;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        if (!IsEnabled)
        {
            return ExerciseMediaMatchResult.Skipped("ExerciseDB fallback media enrichment is disabled.");
        }

        var entries = await EnsureEntriesLoadedAsync(cancellationToken);
        if (entries.Count == 0)
        {
            return ExerciseMediaMatchResult.NotFound("ExerciseDB returned no media entries.");
        }
        ...
        {
            return ExerciseMediaMatchResult.NotFound("ExerciseDB has no usable media entries for the requested media type.");
        }
        ...
        if (exactMatches.Count > 1)
        {
            ...
            return ExerciseMediaMatchResult.Ambiguous("Multiple ExerciseDB entries matched the normalized exercise name.", 0.99);
        }
        ...
        var rankedMatches = eligibleEntries
            .Select(entry => new RankedExerciseDbMatch(entry, ScoreCandidate(item, entry)))
        ...
        {
            return ExerciseMediaMatchResult.NotFound("No high-confidence ExerciseDB fallback match was found.");
        }
        ...
        if (bestMatch.Score >= 0.9 && secondBestScore >= 0.87 && bestMatch.Score - secondBestScore < 0.03)
        {
            return ExerciseMediaMatchResult.Ambiguous(
                "ExerciseDB produced multiple plausible high-confidence matches, so enrichment was skipped.",
                bestMatch.Score);
        }

        return ExerciseMediaMatchResult.LowConfidence(
            "Best ExerciseDB fallback match was below the confidence threshold.",
            bestMatch.Score,
            bestMatch.Entry.Name);
    }

    private async Task<IReadOnlyList<ExerciseDbMediaEntry>> EnsureEntriesLoadedAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(exception, "Failed to fetch ExerciseDB fallback exercise data from {Path}.", _options.ExercisePath);
            throw;
        }
    ...
    private ExerciseDbMediaEntry? MapEntry(JsonElement item)
    {
        ...
        return new ExerciseDbMediaEntry
        {
            Name = name,
        ...
        string message)
    {
        ...
        {
            return ExerciseMediaMatchResult.NotFound("Matched ExerciseDB entry has no usable media for the requested fields.");
        }

        return ExerciseMediaMatchResult.Found(imageUrl, videoUrl, confidence, message, entry.Name);
    }

    private static List<ExerciseDbMediaEntry> DisambiguateExactMatches(
        ExerciseCatalogItem item,
        IEnumerable<ExerciseDbMediaEntry> matches)
    {
    ...
    private static double ScoreCandidate(ExerciseCatalogItem item, ExerciseDbMediaEntry entry)
    {
    ...
    private static bool MatchesMuscle(ExerciseCatalogItem item, ExerciseDbMediaEntry entry)
    {
        var primaryMuscle = ExerciseCatalogMediaEnrichmentService.NormalizeName(item.PrimaryMuscle);
    ...
    private sealed class ExerciseDbMediaEntry
    {
    ...
    private sealed record RankedExerciseDbMatch(ExerciseDbMediaEntry Entry, double Score);
}
